...
if (empty($_SESSION['username']) AND empty($_SESSION['passuser'])) {
	...
	if($nama_file==""){
	$querySimpan = mysqli_query($koneksi,"UPDATE kopi SET nama_kopi='$namaKopi', stok='$stok', deskripsi='$deskripsi', harga='$harga' WHERE id_kopi='$idKopi'");
	} else {
		   move_uploaded_file($tmp_file, $path);// Cek apakah gambar berhasil diupload atau tidak
                // Jika gambar berhasil diupload, Lakukan :
                // Proses simpan ke Database
          $querySimpan = mysqli_query("UPDATE kopi SET nama_kopi='$namaKopi', stok='$stok', gambar='$nama_file', deskripsi='$deskripsi', harga='$harga' WHERE id_kopi='$idKopi'");
			}
				if($querySimpan){
